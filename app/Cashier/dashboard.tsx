  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.statCard}>
          <Text style={[styles.statNumber, { color: '#9B59B6' }]}>{withDoctorPatients.length}</Text>
          <Text style={styles.statLabel}>With Doctor</Text>
        </View>
        <View style={styles.statCard}>
          <Text style={[styles.statNumber, { color: '#2ECC71' }]}>{paidPatients.length}</Text>
          <Text style={styles.statLabel}>Paid</Text>
        </View>
        <View style={styles.statCard}>
          <Text style={[styles.statNumber, { color: '#5B8DB8' }]}>{patients.length}</Text>
      ...
      <View style={styles.windowSelector}>
        {windowConfig.map((win, i) => (
          <TouchableOpacity
            ...
            style={[
              styles.windowTab,
              selectedWindow === i && { backgroundColor: win.accent, borderColor: win.accent }
            ]}
      ...
      {/* CURRENT WINDOW CARD */}
      <View style={[styles.currentWindowCard, { borderLeftColor: windowConfig[selectedWindow].accent }]}>
        ...
        {currentPatient ? (
          <>
            ...
            {/* STATUS BADGE */}
            <View style={[styles.statusBadge, currentPatient.status === 'with-doctor'
              ? styles.statusDoctor
              ...
              <Text style={styles.statusText}>
                {currentPatient.status === 'with-doctor' ? '🩺 With Doctor'
                  : currentPatient.status === 'paid' ? '✓ Paid'
                  : '⏳ Waiting'}
              </Text>
            </View>
            ...
            <View style={styles.actionGrid}>
              <TouchableOpacity
                ...
                onPress={() => router.push({
                  ...
                  params: { patient: JSON.stringify(currentPatient), windowIndex: selectedWindow }
                })}
                activeOpacity={0.85}
              >
      ...
      {withDoctorPatients.length > 0 && (
        <>
          ...
          <View style={styles.sectionRow}>
            ...
            <Text style={styles.sectionLabel}>WITH DOCTOR ({withDoctorPatients.length})</Text>
          </View>
          <FlatList
            ...
            scrollEnabled={false}
            renderItem={({ item }) => (
              <View style={[styles.queueItem, { borderLeftWidth: 3, borderLeftColor: '#9B59B6' }]}>
                <View style={[styles.queueBadge, { backgroundColor: '#6C3483' }]}>
                  <Text style={styles.queueNumber}>{item.queue}</Text>
                </View>
                <Text style={styles.queueName}>{item.name}</Text>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={[styles.queueItem, { opacity: 0.6 }]}>
                <View style={[styles.queueBadge, { backgroundColor: '#1E8449' }]}>
                  <Text style={styles.queueNumber}>{item.queue}</Text>
                </View>
                <Text style={styles.queueName}>{item.name}</Text>
...
const styles = StyleSheet.create({
  ...
  headerSub: { fontSize: 10, fontWeight: '600', color: '#5B8DB8', letterSpacing: 3, marginBottom: 4 },
  ...
  statCard: {
    flex: 1, backgroundColor: '#0F2744', borderRadius: 10,
    padding: 14, alignItems: 'center', borderWidth: 1, borderColor: '#1E3A5F',
  },
  statNumber: { fontSize: 26, fontWeight: '700', color: '#E67E22' },
  statLabel: { fontSize: 10, color: '#5B8DB8', fontWeight: '600', marginTop: 2, letterSpacing: 1 },
  ...
  // Current window card
  currentWindowCard: {
    backgroundColor: '#0F2744', borderRadius: 12,
  ...
  cardPatient: { fontSize: 24, fontWeight: '700', color: '#FFFFFF', marginBottom: 2 },
  cardPatientEmpty: { color: '#2E4F6E', fontStyle: 'italic', fontWeight: '400', fontSize: 17, marginBottom: 12 },
  cardQueue: { fontSize: 12, color: '#5B8DB8', marginBottom: 10 },

  // Status badge
  statusBadge: { alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 5, borderRadius: 12, marginBottom: 14 },
  ...
  removeBtn: {
    ...
    paddingVertical: 9, alignItems: 'center', marginBottom: 14,
  },
  removeBtnText: { color: '#E74C3C', fontSize: 13, fontWeight: '600' },

  nextButton: {
    backgroundColor: '#2471A3', borderRadius: 8,
  ...
  doctorBadge: { backgroundColor: '#6C3483', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4 },
  doctorBadgeText: { fontSize: 13 },

  paidBadge: { backgroundColor: '#1E8449', borderRadius: 6, paddingHorizontal: 8, paddingVertical: 4 },
  ...
  logoutButtonText: { color: '#FFFFFF', fontSize: 15, fontWeight: '700', letterSpacing: 0.5 },
});